...
class SGNSDataset(Dataset):
    def __init__(self, corpus, vocab, context_size=2, n_negatives=5, ns_dist=None):
        ...
        for sentence in tqdm(corpus, desc="Dataset Construction"):
            ...
            for i in range(1, len(sentence)-1):
                ...
                context += [self.pad] * (2 * context_size - len(context)) # 进行pad
    ...
    def collate_fn(self, examples):
        ...
        for i in range(batch_size):
            ...
            ns_dist = self.ns_dist.index_fill(0, contexts[i], .0) # 对第0维contexts位置填充0.0
            # 对每一行进行（self.n_negatives * context_size）次有放回的取样，contexts位置为0，所以负样本永远采样不到当前样本中的context
            neg_contexts.append(torch.multinomial(ns_dist, self.n_negatives * context_size, replacement=True))
        neg_contexts = torch.stack(neg_contexts, dim=0) # 把list中tensor列表堆叠并整体转成tensor类型
...
for epoch in range(num_epoch):
    ...
    for batch in tqdm(data_loader, desc=f"Training Epoch {epoch}"):
        ...
        # 正样本的分类（对数）似然
        # torch.bmm:matrix-matrix product, must be 3D tensor, ans_shape=(batch_size,context_num,1)
        context_loss = F.logsigmoid(torch.bmm(context_embeds, word_embeds).squeeze(dim=2))
        context_loss = context_loss.mean(dim=1)
        # 负样本的分类（对数）似然
        neg_context_loss = F.logsigmoid(torch.bmm(neg_context_embeds, word_embeds).squeeze(dim=2).neg()) # (batch_size,n_negatives * context_size,1)
