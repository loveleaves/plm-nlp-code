...
def load_corpus(path, max_tok_len=None, max_seq_len=None):
    ...
    with codecs.open(path, "r", encoding="utf-8") as f: # codecs好处：不会因为编码问题出错
        for line in tqdm(f):
...
class BiLMDataset(Dataset):
    ...
    def collate_fn(self, examples):
        ...
        for i, (sent_w, sent_c) in enumerate(examples):
            targets_fw[i][:len(sent_w)-1] = torch.LongTensor(sent_w[1:]) # forward output
            targets_bw[i][1:len(sent_w)] = torch.LongTensor(sent_w[:len(sent_w)-1]) # backward output
